			...
			WHERE p_name LIKE '%$query%' OR p_description LIKE '%$query%'") or die(mysqli_error($link));
				...
				if (mysqli_num_rows($raw_results) > 0) { // if one or more rows are returned do following
					while ($results = mysqli_fetch_array($raw_results)) {
						...
						<img class="card-img-top" src="' . $results['p_image'] . '" style=" display: block; margin-left: auto; margin-right: auto;max-width: 150px; max-height:250px; overflow: hidden; object-position: 50% 50%; object-fit: contain;" alt="' . $results['p_name'] . '">
						<h5 class="card-title">' . $results['p_name'] . '</h5>
						</div>
						</a>
						</div>';
